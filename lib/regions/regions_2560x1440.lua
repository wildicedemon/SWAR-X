-- ========================
-- 	Regions for resolution
-- 		2560 x 1440
-- ========================
upperRight              = Region(1900, 0, 500, 440)
right                   = Region(1284, 500, 960, 560)
left                    = Region(300, 500, 960, 560)
bottomLeft              = Region(0, 880, 684, 556)

buttonRegion            = Region(768, 964, 1028, 428)
rechargeEnergy          = Region(400,400,650,550)

runeSnap                = Region(700,0,1150,1440)
statsSection            = Region(2160,400,400,600)

--StageList Loop Regions
battleGearWheelReg      = Region(0,1220,225,221)
victoryDiamondReg       = Region(1800,580,350,300)
victoryRaidDamageReg    = Region(170,610,370,120)
worldMapReg             = Region(1632,1040,440,440)
bigFlashReg             = Region(2000,855,250,230)
arenaBigWingReg         = Region(2000,855,250,230)
playReg                 = Region(368,1220,225,221)
pauseReg                = Region(1050,537,460,187)
arenaResultsReg         = Region(320,800,250,250)
bigCancalReg            = Region(0,0,2560,1440)

battleButtonReg         = Region(1275,1150,335,290)
areaMapReg              = Region(2225,1150,335,290)
dungeonStageReg         = Region(300,350,800,1050)

yesWordPngReg           = Region(798,751,530,263)
okenReg                 = Region(1026,949,503,310)

cancelCrossReg          = Region(1575,95,305,585)
cancel2Reg              = Region(1577,190,300,800)
cancelLongReg           = Region(1050,150,800,400)
cancelRefillReg         = Region(2031,91,208,219)

--Ally Monster Regions
TopMon                  = Region(492,232,320,320)
LeftMon                 = Region(220,376,320,320)
RightMon                = Region(764,376,320,320)
BottomMon               = Region(492,520,320,320)

--Arena Monster/Enemy Regions
eTopMon                 = Region(1760,224,300,300)
eLeftMon                = Region(1500,370,300,300)
eBottomMon              = Region(1760,512,300,300)
eRightMon               = Region(2020,370,300,300)

--(Fodder) Leveing Monster Regions
NewFodder               = Region(0,780,1800,380)
EndofMonL               = Region(1660,780,400,460)
monListLeft             = Location(120,1000)
monListRight            = Location(1660,1000)
TopMonEXP               = Region(600,890,460,220)
LeftMonEXP              = Region(1100,890,460,220)
RightMonEXP             = Region(1620,890,460,220)
BottomMonEXP            = Region(580,1062,460,220)
FifthMonEXP             = Region(1100,1062,460,220) -- UNUSED
SixthMonEXP             = Region(1620,1062,460,220) -- UNUSED

--Arena Battle Regions
arenaOppReg             = Region(1700,150,450,1200)
EndofArenaL             = Region(1550,925,650,425)
arenaRefreshReg         = Region(1550,150,600,225)
arenaRefreshWaitReg     = Region(850,180,840,220)
arenaRefreshListReg     = Region(850,635,850,300)
arenaDialogReg          = Region(0,1011,190,190)
arenaSelectNormalReg    = Region(480,250,780,930)
arenaSelectWorldReg     = Region(1300,250,780,930)
arenaBattleLogReg       = Region(1275,1150,650,290)
arenaButtonReg          = Region(1700,330,860,760)
--Rune Evaluation Regions
runeStarRegion          = Region(755,377,350,325)
runeStarsRegionD        = Region(850,475,150,150) --Debug Rune Star Display
runeTypeAndSlotRegion   = Region(766,250,1025,300)
runeRarityRegion        = Region(1330,310,450,450)
runePrimeRegion         = Region(930,430,600,120)
runeSubRegion           = Region(770,625,480,325)
runeEvalStats           = Region(1278,620,500,665)
--Needs to be implemented.
runeShopStarRegion      = Region(245,298,350,325) -- UNUSED
runeShopRarityRegion    = Region(255,237,1025,300) -- UNUSED
runeShopPrimeRegion     = Region(419,336,600,120) -- UNUSED
runeShopSubRegion       = Region(255,510,480,350) -- UNUSED